...
- [ ] Add an entry point for tutorials, how-tos, explanations and references (see [Diátaxis](https://diataxis.fr/)) to the [BDI Developer Portal](https://dev.bdinetwork.org/), to complement the European Commission documentation that is geared towards governments and highly regulated enterprises. For example, answer questions such as how to choose between self-hosting versus selecting a provider, where to find providers, which problems does e-delivery solve, which APIs and deployment considerations ot take into account.
- [ ] Deploy a sandbox e-delivery access point with a trust service provider and an example agent API, to enable developers to prototype and demo BDI applications without needing to set up the web server infrastructure.
